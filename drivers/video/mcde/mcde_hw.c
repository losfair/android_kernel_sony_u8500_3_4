...
static void set_channel_state_atomic(struct mcde_chnl_state *chnl,
							enum chnl_state state);
static void set_channel_state_sync(struct mcde_chnl_state *chnl,
							enum chnl_state state);
...
static int wait_for_vcmp(struct mcde_chnl_state *chnl, int vcmp_count);
...
const char *mcde_channel_state_to_string(enum chnl_state state)
{
	switch (state) {
...
static void get_dsi_port(struct mcde_chnl_state *chnl,
		...
		struct mcde_chnl_state *chnl)
{
	...
	if (chnl->port.sync_src != MCDE_SYNCSRC_BTA) {
		dev_err(&mcde_dev->dev, "Unsupported sync_src");
		return;
	}

	n_listeners = atomic_inc_return(&chnl->n_bta_te_oneshot_listeners);
	trace_keyvalue((0xF0 | chnl->id), "mcde_add_bta_te_oneshot_listener",
			n_listeners);
	if (n_listeners == 1)
		nova_dsilink_te_request(chnl->dsilink);
}

static inline void mcde_remove_bta_te_oneshot_listener(
			struct mcde_chnl_state *chnl)
{
	int n_listeners = atomic_dec_return(&chnl->n_bta_te_oneshot_listeners);

	trace_keyvalue((0xF0 | chnl->id), "mcde_remove_bta_te_oneshot_listener",
			n_listeners);
}
...
static void handle_dsi_irq(struct mcde_chnl_state *chnl)
{
	...
	if (events & DSILINK_IRQ_BTA_TE) {
		...
		case CHNLSTATE_WAIT_TE:
			...
			} else if (chnl->port.frame_trig == MCDE_TRIG_HW) {
				set_channel_state_atomic(chnl,
						CHNLSTATE_RUNNING);
				set_channel_state_atomic(chnl,
						CHNLSTATE_STOPPING);
			} else
		...
		if (chnl->is_bta_te_listening == true) {
			mcde_remove_bta_te_oneshot_listener(chnl);
	...
	if (events & DSILINK_IRQ_NO_TE) {
		dev_err(&mcde_dev->dev,	"DSILINK_IRQ_NO_TE. events=0x%08x",
				events);
	}
...
static void mcde_register_vcmp(struct mcde_chnl_state *chnl)
{
	if (!chnl->vcmp_per_field ||
			(chnl->vcmp_per_field && chnl->even_vcmp)) {
		chnl->vcmp_timestamp = ktime_get();
		atomic_inc(&chnl->vcmp_cnt);
		chnl->vsync_cnt_wait = atomic_read(&chnl->vsync_cnt) + 1;
	}
}

static inline void handle_chnl_error(void)
{
	u32 irq_status = mcde_rreg(MCDE_MISCHNL);

	trace_chnl_err(irq_status);

	/* Check for channel abort and restart correct channel if so */
	if (irq_status & MCDE_MISCHNL_CHNLAMIS_MASK) {
		int chnl_i;
		u32 mask;
		for (chnl_i = 0; chnl_i <= num_channels; chnl_i++) {
			mask = (1 << (MCDE_MISCHNL_CHNLAMIS_SHIFT + chnl_i));
			if (irq_status & mask) {
				dev_err(&mcde_dev->dev,
						"Channel Abort. chnl=%d\n",
						chnl_i);
				atomic_set(&channels[chnl_i].force_restart,
						true);
			...
			enum chnl_state state)
{
	...
	dev_dbg(&mcde_dev->dev, "Channel state change"
		" (chnl=%d, old=%d (%s), new=%d (%s))\n", chnl->id,
		chnl_state, mcde_channel_state_to_string(chnl_state),
		state, mcde_channel_state_to_string(state));
			...
			enum chnl_state state)
{
	...
	dev_dbg(&mcde_dev->dev, "Channel state change"
		" (chnl=%d, old=%d (%s), new=%d (%s))\n", chnl->id,
		chnl->state, mcde_channel_state_to_string(chnl->state),
		state, mcde_channel_state_to_string(state));
	...
	if (chnl_state != CHNLSTATE_IDLE) {
		...
		if (WARN_ON_ONCE(!ret))
			dev_err(&mcde_dev->dev, "Wait for channel timeout "
						"(chnl=%d, curr=%d, new=%d)\n",
		...
		s64 *timestamp)
{
	long rem_jiffies;
	int w;
	ktime_t call_timestamp;
	bool wait_for_next_vcmp;

	call_timestamp = ktime_get();
	...
	if (chnl->port.mode == MCDE_PORTMODE_VID) {
		if (chnl->port.sync_src == MCDE_SYNCSRC_OFF) {
			wait_for_next_vcmp = true;
			w = atomic_read(&chnl->vcmp_cnt) + 1;
		} else {
			mcde_unlock(__func__, __LINE__);
			return -EIO;
		}
	} else {
		wait_for_next_vcmp = false;
		 ...
		 */
		switch (chnl->port.sync_src) {
		case MCDE_SYNCSRC_TE0: /* Intentional */
		case MCDE_SYNCSRC_TE1:
		...
		{
			/*
			 * Requesting a BTA TE sets the DSI link to listening
			 * mode and blocks sending of new data to the
			 * display and therefore blocks updates of the display.
			 * But if this function is called continuously from a
			 * different task than update, we must yield to let the
			 * update take place.
			 * This can be done since we can delay the waiting
			 * of the BTA TE to a time before the expected BTA TE.
			 */
			s64 diff_us;

			diff_us = ktime_us_delta(call_timestamp,
					chnl->vsync_timestamp);
			if (diff_us < 12*USEC_PER_MSEC) {
				u32 sleep_us = 12*USEC_PER_MSEC - diff_us;
				...
				/* Wake up MCDE if gone disabled during sleep */
				if (enable_mcde_hw()) {
					mcde_unlock(__func__, __LINE__);
					return -EIO;
				}
				if (!chnl->formatter_updated)
					(void)update_channel_static_registers(
							chnl);
				...
				if (diff_us > 16*USEC_PER_MSEC)
					WARN_ON(true);
			}

			if (chnl->state != CHNLSTATE_WAIT_TE) {
				/*
				 * Enter CHNLSTATE_REQ_BTA_TE state that blocks
				 * updates of the display. This state is hold
				 * until BTA TE is received.
				 * Skip this if channel is already
				 * waiting for TE.
				 */
				set_channel_state_sync(chnl,
						CHNLSTATE_REQ_BTA_TE);
			}
			mcde_add_bta_te_oneshot_listener(chnl);
			break;
		}
		case MCDE_SYNCSRC_OFF: /* Intentional */
		case MCDE_SYNCSRC_TE_POLLING: /* Intentional */
		default:
			mcde_unlock(__func__, __LINE__);
			return -EIO;
		}
	...
	if (wait_for_next_vcmp) {
		trace_keyvalue((0xF0 | chnl->id), "Wait for next vcmp", w);
		rem_jiffies = wait_event_timeout(chnl->vcmp_waitq,
	...
	/*
	 * The timestamp argument should be monotonic time in ns.
	 * The value is set in the ISR.
	 * Does not need spinlock protection since
	 */

	if (timestamp != NULL) {
		if (wait_for_next_vcmp)
			*timestamp = ktime_to_ns(chnl->vcmp_timestamp);
		else
			*timestamp = ktime_to_ns(chnl->vsync_timestamp);
	}

	if (wait_for_next_vcmp == false) {
		switch (chnl->port.sync_src) {
		case MCDE_SYNCSRC_TE0:
			/* Intentional */
		case MCDE_SYNCSRC_TE1:
	...
	if (rem_jiffies == 0) {
		...
		dev_err(&mcde_dev->dev,
				"%s. Timeout for %s: chnl=%d, state=%d\n"
				, __func__,
				wait_for_next_vcmp ? "vcmp" : "vsync", chnl->id,
						chnl->state);
		return -1;
	} else {
		trace_keyvalue((0xF0 | chnl->id), "Done waiting", w);
	}
	return 0;
}
...
static void stop_channel(struct mcde_chnl_state *chnl)
{
	...
	if (chnl->port.sync_src == MCDE_SYNCSRC_OFF) {
		disable_flow(chnl, true);
		/*
		 * Needs to manually trigger VCMP after the channel is
...
exit_check:
	if (atomic_read(&chnl->n_bta_te_oneshot_listeners) != 0) {
		dev_err(&mcde_dev->dev, "n_bta_te_oneshot_listeners!=0 (%d)\n",
				atomic_read(&chnl->n_bta_te_oneshot_listeners));
		atomic_set(&chnl->n_bta_te_oneshot_listeners, 0);
	}
	if (atomic_read(&chnl->n_vsync_capture_listeners) != 0) {
		dev_err(&mcde_dev->dev, "n_vsync_capture_listeners!=0 (%d)\n",
				atomic_read(&chnl->n_vsync_capture_listeners));
		atomic_set(&chnl->n_vsync_capture_listeners, 0);
	}
	...
	case MCDE_CHNL_A:
		if (WARN_ON_ONCE(!mcde_rfld(MCDE_CTRLA, FIFOEMPTY)))
			dev_warn(&mcde_dev->dev, "Fifo not empty chnl=%d\n",
					chnl->id);
		...
		break;
	case MCDE_CHNL_B:
		if (WARN_ON_ONCE(!mcde_rfld(MCDE_CTRLB, FIFOEMPTY)))
			dev_warn(&mcde_dev->dev, "Fifo not empty chnl=%d\n",
					chnl->id);
		if (WARN_ON_ONCE(mcde_rfld(MCDE_CRB0, FLOEN)))
			dev_warn(&mcde_dev->dev, "FLOEN at stop chnl=%d\n",
					chnl->id);
		break;
	case MCDE_CHNL_C0:
		if (WARN_ON_ONCE(!mcde_rfld(MCDE_CTRLC0, FIFOEMPTY)))
			dev_warn(&mcde_dev->dev, "Fifo not empty chnl=%d\n",
					chnl->id);
		if (WARN_ON_ONCE(mcde_rfld(MCDE_CRC, C1EN)))
			dev_warn(&mcde_dev->dev, "FLOEN at stop chnl=%d\n",
					chnl->id);
		break;
	case MCDE_CHNL_C1:
		if (WARN_ON_ONCE(!mcde_rfld(MCDE_CTRLC1, FIFOEMPTY)))
			dev_warn(&mcde_dev->dev, "Fifo not empty chnl=%d\n",
					chnl->id);
		if (WARN_ON_ONCE(mcde_rfld(MCDE_CRC, C2EN)))
			dev_warn(&mcde_dev->dev, "FLOEN at stop chnl=%d\n",
					chnl->id);
		break;
	default:
...
static int enable_mcde_hw(void)
{
	...
	for (i = 0; i < num_channels; i++) {
		...
		if (chnl->state == CHNLSTATE_SUSPEND) {
			...
			chnl->vcmp_cnt_wait_return = 0;
			chnl->vsync_timestamp = ktime_set(0, 0);
			chnl->vcmp_timestamp = ktime_set(0, 0);
			atomic_set(&chnl->n_vsync_capture_listeners, 0);
			atomic_set(&chnl->n_bta_te_oneshot_listeners, 0);
			chnl->is_bta_te_listening = false;
		}
				...
				u8 cmd, u8 *data, int len)
{
	...
	mcde_wreg(MCDE_CHNL0SYNCHMOD +
			chnl->id * MCDE_CHNL0SYNCHMOD_GROUPOFFSET,
			...
			bool dcs, u8 cmd, u8 *data, int len)
{
	...
	if ((len <= DSILINK_MAX_DSI_DIRECT_CMD_WRITE && !dcs) ||
		...
		/*
		 * Some panels don't allow commands during update in command
		 * mode. Issue not seen on video mode panels, so we let
		 * DSI link do the arbitration on packet level.
		 */
		if (chnl->port.mode == MCDE_PORTMODE_CMD)
			set_channel_state_sync(chnl, CHNLSTATE_DSI_WRITE);
...
static inline u32 mcde_to_oled_pal(u16 l, u16 r)
{
	return ((l & 0x0FFF) << 12) | (r & 0x0FFF);
}

static void chnl_update_oled_palette(struct mcde_chnl_state *chnl,
		enum mcde_chnl chnl_id)
{
	if (!chnl->update_oled_palette)
		return;
	...
	if (chnl_id == MCDE_CHNL_A || chnl_id == MCDE_CHNL_B) {
		...
		chnl->regs.oled_map_b = chnl->oled_map_b;
		chnl->regs.oled_map_g = chnl->oled_map_g;
		chnl->regs.oled_map_r = chnl->oled_map_r;
		chnl->regs.oled_palette_context = chnl->oled_palette_context;

		chnl->regs.oled_palette_enable = chnl->oled_palette_enable;

		if (chnl_id == MCDE_CHNL_A) {
			mcde_gam_b = MCDE_GAM0A;
			mcde_gam_g = MCDE_GAM1A;
			mcde_gam_r = MCDE_GAM2A;
			mcde_wfld(MCDE_CRA0, GAMEN, chnl->regs.oled_palette_enable);
		} else {
			mcde_gam_b = MCDE_GAM0B;
			mcde_gam_g = MCDE_GAM1B;
			mcde_gam_r = MCDE_GAM2B;
			mcde_wfld(MCDE_CRB0, GAMEN, chnl->regs.oled_palette_enable);
		}

		if (chnl->regs.oled_palette_enable) {

			int i;
			void *context = chnl->regs.oled_palette_context;

			for (i = 0; i < 32; i++) {

				u32 red = mcde_to_oled_pal(
					chnl->regs.oled_map_r(0, i, context),
					chnl->regs.oled_map_r(1, i, context));
				u32 green = mcde_to_oled_pal(
					chnl->regs.oled_map_g(0, i, context),
					chnl->regs.oled_map_g(1, i, context));
				u32 blue = mcde_to_oled_pal(
					chnl->regs.oled_map_b(0, i, context),
					chnl->regs.oled_map_b(1, i, context));

				mcde_wreg(mcde_gam_b, MCDE_GAM0A_BLUE(blue));
				mcde_wreg(mcde_gam_g, MCDE_GAM1A_GREEN(green));
				mcde_wreg(mcde_gam_r, MCDE_GAM2A_RED(red));
			}
		}
	}

	chnl->update_oled_palette = false;
}
...
static void setup_channel(struct mcde_chnl_state *chnl)
{
	...
	chnl_update_oled_palette(chnl, chnl->id);
}
...
static int _mcde_chnl_update(struct mcde_chnl_state *chnl)
{
	...
	if (!chnl->port.update_auto_trig || chnl->state != CHNLSTATE_RUNNING) {
		...
		(void)wait_for_vcmp(chnl, chnl->vcmp_cnt_wait);
		curr_vcmp_cnt = atomic_read(&chnl->vcmp_cnt);
	} else {
	...
	if (WARN_ON(curr_vcmp_cnt !=  atomic_read(&chnl->vcmp_cnt)))
		dev_err(&mcde_dev->dev,
				"MCDE register writes was not atomic. %d!=%d",
				curr_vcmp_cnt, atomic_read(&chnl->vcmp_cnt));

	/* Make sure previous update is handled before returning */
	if (!chnl->port.update_auto_trig || chnl->state != CHNLSTATE_RUNNING) {
		/* Command mode or video mode stopped */
	} else if (chnl->port.sync_src == MCDE_SYNCSRC_TE0 ||
		   chnl->port.sync_src == MCDE_SYNCSRC_TE1) {
		/* Running video mode with vsync */
		(void)wait_for_vcmp(chnl, chnl->vcmp_cnt_wait_return);
		chnl->vcmp_cnt_wait_return = curr_vcmp_cnt + 1;
	} else {
		/* Running video mode without vsync */
		(void)wait_for_vcmp(chnl, chnl->vcmp_cnt_wait_return);
		chnl->vcmp_cnt_wait_return = curr_vcmp_cnt + 2;
	}
					...
					struct mcde_oled_palette_table *palette)
{
	...
	if (palette != NULL) {
		chnl->oled_palette_context = palette->context;
		chnl->oled_map_r = palette->map_col_ch0;
		chnl->oled_map_g = palette->map_col_ch1;
		chnl->oled_map_b = palette->map_col_ch2;
		chnl->oled_palette_enable = true;
	} else {
		chnl->oled_palette_enable = false;
		chnl->oled_palette_context = NULL;
		chnl->oled_map_r = NULL;
		chnl->oled_map_g = NULL;
		chnl->oled_map_b = NULL;
	}
