...
static bool debugfs_req ;
static s32 arm_freq_treshold;
static s32 arm_max_freq;
...
static int __prcmu_qos_update_requirement(int prcmu_qos_class, const char *name,
...
static struct prcmu_qos_object arm_khz_qos = {
	 ...
	 */
	.name = "arm_khz",
	/* .default_value and .max_value are set during the init */
	/* Notice that arm is in kHz, not in % */
	.force_value = 0,
	.comparitor = max_compare
};

static struct prcmu_qos_object arm_max_freq_qos = {
	.requirements = {
		LIST_HEAD_INIT(arm_max_freq_qos.requirements.list)
	},
	.name = "arm_max_freq",
	/* .default_value and .max_value are set during the init */
	.force_value = 0,
...
static const char debugfs_req_name[] = "debug_arm_max_freq";
...
void update_cpu_max_freq(void)
{

	int cpu;
	struct cpufreq_policy policy;
	s32 max_freq, min_freq;
	int ret;

	if (!prcmu_check_ape_age())
		max_freq = arm_freq_treshold;
	else
		max_freq = arm_max_freq;

	max_freq = min(max_freq, prcmu_qos_requirement(PRCMU_QOS_ARM_MAX_FREQ));
	...
	for_each_online_cpu(cpu) {
		...
		/*
		 * Before setting the new max_freq value, we have to check
		 * if this value is lower than current min_freq.
		 * If it is, current min_freq has to be updated before max_freq
		 * and in that case min_freq = max_freq.
		 */
		min_freq = policy.min;
		if (max_freq < min_freq) {
			...
			if (ret) {
				pr_err("prcmu qos: update frequency limits "
						"failed\n");
				return;
			}
		}

		if ((min_freq != policy.min) || (max_freq != policy.max)) {
			ret = cpufreq_update_freq(cpu, min_freq, max_freq);
			if (ret)
				pr_err("prcmu qos: update frequency limits failed\n");
		}
	}
}
#else
static inline void update_cpu_max_freq(void) { }
#endif
...
static void update_cpu_limits(s32 min_freq)
{
	...
	for_each_online_cpu(cpu) {
		...
		if (min_freq != policy.min) {
			ret = cpufreq_update_freq(cpu, min_freq, policy.max);
			if (ret)
				pr_err("prcmu qos: update cpufreq frequency limits failed\n");
		}
	}
}
#else
...
static void update_target(int target, bool sem)
{
	...
	case PRCMU_QOS_ARM_KHZ:
		...
		/* check if arm OPP max is older than expected*/
		if (prcmu_check_ape_age())
			update_cpu_max_freq();
...
void prcmu_qos_force_opp(int prcmu_qos_class, s32 val)
{

	if (prcmu_qos_class == PRCMU_QOS_ARM_MAX_FREQ) {
		if (cpu_is_ux540_family()) {
			if (debugfs_req == false) {
				prcmu_qos_add_requirement(prcmu_qos_class,
						debugfs_req_name , val);
				debugfs_req = true;
			} else
				prcmu_qos_update_requirement(prcmu_qos_class,
					debugfs_req_name, val);
		}
	} else {
		prcmu_qos_array[prcmu_qos_class]->force_value = val;
 ...
 */
static int __prcmu_qos_add_requirement(int prcmu_qos_class, const char *name,
		s32 value, bool sem)
{
...
int prcmu_qos_add_requirement(int prcmu_qos_class, const char *name, s32 val)
{
...
int prcmu_qos_update_requirement(int prcmu_qos_class, const char *name,
		s32 val)
{
 ...
 */
static void __prcmu_qos_remove_requirement(int prcmu_qos_class, const char *name,
		bool sem)
{
	unsigned long flags;
	struct requirement_list *node;
...
void prcmu_qos_remove_requirement(int prcmu_qos_class, const char *name)
{
...
void prcmu_qos_show_requirement(struct seq_file *s, u32 verbose)
{
	...
	seq_printf(s, "--PRCMU_QOS_APE_OPP\n");
	list_for_each_entry(node, &prcmu_qos_array[
			PRCMU_QOS_APE_OPP]->requirements.list, list) {
		if (verbose || (node->value != prcmu_qos_array[
				PRCMU_QOS_APE_OPP]->default_value))
			seq_printf(s, "	%s , OPP%u%%\n", node->name,
					node->value);
	}

	seq_printf(s, "--PRCMU_QOS_DDR_OPP\n");
	list_for_each_entry(node, &prcmu_qos_array[
			PRCMU_QOS_DDR_OPP]->requirements.list, list) {
		if (verbose || (node->value != prcmu_qos_array[
				PRCMU_QOS_DDR_OPP]->default_value))
			seq_printf(s, "	%s , OPP%u%%\n", node->name,
					node->value);
	}

	seq_printf(s, "--PRCMU_QOS_VSAFE_OPP\n");
	list_for_each_entry(node, &prcmu_qos_array[
			PRCMU_QOS_VSAFE_OPP]->requirements.list, list) {
		if (verbose || (node->value != prcmu_qos_array[
				PRCMU_QOS_VSAFE_OPP]->default_value))
			seq_printf(s, "	%s , OPP%u%%\n", node->name,
					node->value);
	}

	seq_printf(s, "--PRCMU_QOS_ARM_KHZ\n");
	list_for_each_entry(node, &prcmu_qos_array[
			PRCMU_QOS_ARM_KHZ]->requirements.list, list) {
		if (verbose || (node->value != prcmu_qos_array[
				PRCMU_QOS_ARM_KHZ]->default_value))
			seq_printf(s, "	%s , %u khz\n", node->name,
					node->value);
	}

	seq_printf(s, "--PRCMU_QOS_ARM_MAX_FREQ\n");
	list_for_each_entry(node, &prcmu_qos_array[
			PRCMU_QOS_ARM_MAX_FREQ]->requirements.list, list) {
		if (verbose || (node->value != prcmu_qos_array[
				PRCMU_QOS_ARM_MAX_FREQ]->default_value))
			seq_printf(s, "	%s , %u khz\n", node->name,
					node->value);
	}
}
...
static int prcmu_qos_arm_max_freq_power_open(struct inode *inode,
		struct file *filp)
{
	return prcmu_qos_power_open(inode, filp, PRCMU_QOS_ARM_MAX_FREQ);
}
...
static const struct file_operations prcmu_qos_arm_max_freq_power_fops = {
	.write = prcmu_qos_power_write,
	.open = prcmu_qos_arm_max_freq_power_open,
	.release = prcmu_qos_power_release,
};
...
static int __init prcmu_qos_power_early_init(void)
{
	if (cpu_is_ux540_family())
		update_target_max_recur = 2;

	/* Ensure 100% DDR and APE OPP during boot */
	prcmu_qos_add_requirement(PRCMU_QOS_APE_OPP,
			"qos-boot-ape", PRCMU_QOS_MAX_VALUE);

	prcmu_qos_add_requirement(PRCMU_QOS_DDR_OPP,
			"qos-boot-ddr", PRCMU_QOS_MAX_VALUE);

	return 0;
}

static int __init prcmu_qos_power_late_init(void)
{
	int ret;
	...
	for (i = 0; table[i].frequency != CPUFREQ_TABLE_END; i++) {
		...
		if (max_freq < table[i].frequency) {
			/*
			 * freq threshold should be the frequency
			 * just below max_freq
			 */
			arm_freq_treshold = max_freq;
	...
	arm_max_freq = max_freq;

	prcmu_qos_cpufreq_init_done = true;

	arm_max_freq_qos.max_value = max_freq;
	arm_max_freq_qos.default_value = max_freq;
	atomic_set(&arm_max_freq_qos.target_value, arm_max_freq_qos.max_value);
	debugfs_req = false;
	...
	if (cpu_is_ux540_family()) {
		ret = register_prcmu_qos_misc(&arm_max_freq_qos,
				&prcmu_qos_arm_max_freq_power_fops);
		if (ret < 0) {
			pr_err("prcmu arm max freq qos: setup failed\n");
			goto arm_max_qos_error;
		}
	...
	/* Remove requirements and possibly return to default OPP*/
	prcmu_qos_remove_requirement(PRCMU_QOS_APE_OPP, "qos-boot-ape");

	prcmu_qos_remove_requirement(PRCMU_QOS_DDR_OPP, "qos-boot-ddr");

	return ret;
vsafe_opp_qos_error:
	misc_deregister(&arm_max_freq_qos.prcmu_qos_power_miscdev);
arm_max_qos_error:
...
arch_initcall(prcmu_qos_power_early_init);
late_initcall(prcmu_qos_power_late_init);
